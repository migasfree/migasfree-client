...
class TestErrorHandling:
    ...
    def test_run_http_404_error(self):
        ...
        responses.add(
            responses.POST, url + "/", json={"error": "Not found"}, status=404
        )
    ...
    def test_run_http_401_unauthorized(self):
        ...
        responses.add(
            responses.POST, url + "/", json={"error": "Unauthorized"}, status=401
        )
...
class TestSafeRequests:
    ...
    def test_run_safe_request(self, mock_unwrap, mock_wrap):
        ...
        result = url_req.run(url, data=data, safe=True, exit_on_error=False)
...
class TestFileUpload:
    ...
    def test_run_with_file_upload(self, mock_remove, mock_write, mock_magic, mock_read):
        ...
        result = url_req.run(
            url, data={}, upload_files=[file_path], safe=False, exit_on_error=False
        )
...
class TestResponseProcessing:
    ...
    def test_evaluate_response_safe(self, mock_unwrap):
        ...
        responses.add(
            responses.POST, url + "/", json={"msg": "encrypted_message"}, status=200
        )
